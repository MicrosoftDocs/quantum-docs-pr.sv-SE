...
En slutgiltig viktig egenskap för mått är att det inte nödvändigt vis skadar alla vektorer med Quantum-tillstånd. Om vi börjar med en qubit i läget $ \begin{bmatrix} 1 \\\\ 0 \end{bmatrix} $, som motsvarar klassiskt läge $0 $, ger detta tillstånd alltid resultatet $0 $ och lämnar Quantum-läget oförändrat. I det här fallet, om vi bara har klassiska bitar (d.v.s. qubits som antingen är $ \begin{bmatrix}1 \\\\ 0 \end{bmatrix} $ eller $ \begin{bmatrix}0 \\\\ 1 \end{bmatrix} $), skadar inte systemet systemet. Det innebär att vi kan replikera klassiska data och ändra dem på en Quantum-dator på samma sätt som ett kan göra på en klassisk dator. Möjligheten att lagra information i båda lägena på en gång är att öka den väntande data bearbetningen utöver vad som är möjligt klassiska och ytterligare Robs quantum datorer i möjligheten att kopiera Quantum-data på ett icke-högre sätt, se även [no-klonings satsen ](https://en.wikipedia.org/wiki/No-cloning_theorem).
...
Observera att eftersom konventionen för Matrix multiplikation är att multiplicera från höger till vänster om den första gaten i den här sekvensen, $G _N $, är den sista som tillämpas på den Quantum-tillstånds vektorn. Mer formellt, anta att en sådan grind uppsättning är universell om för varje fel tolerans $ \epsilon > 0 $ finns $G _1, \ldots, G_N $ så att avståndet mellan $G _N ldots G_1 \epsilon $ och $U $ är högst $ $. Det bästa värdet för $N $ som behövs för att uppnå det här avståndet i $ \epsilon $ bör skala Poly-logarithmically med $1/\ Epsilon $.
...
Den enklaste sådan primitiv är den enda qubit. Tre enkla qubit-rotationer anses vanligt vis: $R _x $, $R _y $ och $R _z $. Om du vill visualisera åtgärden i rotations $R _x (\theta) $, t. ex. att peka ditt högra tumm utmed riktningen på $x $-axeln i Bloch-sfären och rotera vektorn med handen genom en vinkel på $ \ theta/2 $ radianer. Den här förvirrande faktorn på $2 $ uppstår från det faktum att rätvinkliga vektorer är $180 ^ \circ $ isär när de ritas på Bloch-sfären, men som i själva verket är $90 ^ \circ $ grader oberoende av varandra. Motsvarande enhetliga matriser är:
