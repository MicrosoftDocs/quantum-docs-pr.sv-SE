...
- Validera modellen för att fastställa dess exakthet
    ...
    Om du vill köra är du Q # klassificerare från C#, sparar du följande kod som `Host.cs`. Kom ihåg att du även behöver den Q #-fil `Training.qs` som beskrivs senare i den här självstudien.
    ...
    Om du vill köra ditt nya Q # C# -program från i Visual Studio ändrar du `Host.cs` så C# att följande kod ingår. Kom ihåg att du även behöver den Q #-fil `Training.qs` som beskrivs senare i den här självstudien.
...
## <a name="q-classifier-code"></a>F\# klassificerings kod
