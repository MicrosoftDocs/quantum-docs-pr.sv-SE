...
<span data-ttu-id="5ff02-105">I den här självstudien visar vi hur du skriver ett Q# program som manipulerar och mäter qubits och demonstrerar effekterna av superposition och entanglement.</span><span class="sxs-lookup"><span data-stu-id="5ff02-105">In this tutorial, we show you how to write a Q# program that manipulates and measures qubits and demonstrates the effects of superposition and entanglement.</span></span>
...
<span data-ttu-id="5ff02-112">Du kan också följa med i beräkningen utan att installera QDK, granska övervisningen av Q# programmeringsspråket och de första koncepten med Quantum Computing.</span><span class="sxs-lookup"><span data-stu-id="5ff02-112">You can also follow along with the narrative without installing the QDK, reviewing  the overviews of the Q# programming language and the first concepts of quantum computing.</span></span>
...
> * <span data-ttu-id="5ff02-116">Demonstrera Quantum entanglement med ett Q# program som körs i en simulator.</span><span class="sxs-lookup"><span data-stu-id="5ff02-116">Demonstrate quantum entanglement with a Q# program run in a simulator.</span></span> 
...
<span data-ttu-id="5ff02-128">Nu är vi redo att demonstrera hur Q# uttrycker det här beteendet.</span><span class="sxs-lookup"><span data-stu-id="5ff02-128">Now, we're ready to demonstrate how Q# expresses this behavior.</span></span>  <span data-ttu-id="5ff02-129">Du börjar med det enklaste programmet och skapar det för att visa en kvantsuperposition och kvantsammanflätning.</span><span class="sxs-lookup"><span data-stu-id="5ff02-129">You start with the simplest program possible and build it up to demonstrate quantum superposition and quantum entanglement.</span></span>

## <a name="creating-a-no-locq-project"></a><span data-ttu-id="5ff02-130">Skapa ett Q# projekt</span><span class="sxs-lookup"><span data-stu-id="5ff02-130">Creating a Q# project</span></span>

<span data-ttu-id="5ff02-131">Det första vi behöver göra är att skapa ett nytt Q# projekt.</span><span class="sxs-lookup"><span data-stu-id="5ff02-131">The first thing we have to do is to create a new Q# project.</span></span> <span data-ttu-id="5ff02-132">I den här självstudien kommer vi att använda miljön baserat på [ Q# program med vs Code](xref:microsoft.quantum.install.standalone).</span><span class="sxs-lookup"><span data-stu-id="5ff02-132">In this tutorial we are going to use the environment based on [Q# applications with VS Code](xref:microsoft.quantum.install.standalone).</span></span>
...
1. <span data-ttu-id="5ff02-134">Klicka på **Visa** -> **Kommandopalett** och välj **Q#: Skapa nytt projekt** .</span><span class="sxs-lookup"><span data-stu-id="5ff02-134">Click **View** -> **Command Palette** and select **Q#: Create New Project** .</span></span>
...
<span data-ttu-id="5ff02-145">I det första kodfragmentet nedan visar vi dig hur du arbetar med qubits i Q# .</span><span class="sxs-lookup"><span data-stu-id="5ff02-145">In the first code snippet below, we show you how to work with qubits in Q#.</span></span>  <span data-ttu-id="5ff02-146">Vi introducerar två åtgärder [`M`](xref:Microsoft.Quantum.Intrinsic.m) och [`X`](xref:Microsoft.Quantum.Intrinsic.X) som transformerar statusen för en qubit.</span><span class="sxs-lookup"><span data-stu-id="5ff02-146">We’ll introduce two operations, [`M`](xref:Microsoft.Quantum.Intrinsic.m) and [`X`](xref:Microsoft.Quantum.Intrinsic.X) that transform the state of a qubit.</span></span> <span data-ttu-id="5ff02-147">I det här kodfragmentet definieras åtgärden `SetQubitState` som använder en parameter som kvantbit och en annan parameter, `desired`, som representerar det tillstånd som vi vill att kvantbiten ska ha.</span><span class="sxs-lookup"><span data-stu-id="5ff02-147">In this code snippet, an operation `SetQubitState` is defined that takes as a parameter a qubit and another parameter, `desired`, representing the state that we would like the qubit to be in.</span></span>  <span data-ttu-id="5ff02-148">Åtgärden `SetQubitState` utför en mätning på kvantbiten med åtgärden `M`.</span><span class="sxs-lookup"><span data-stu-id="5ff02-148">The operation `SetQubitState` performs a measurement on the qubit using the operation `M`.</span></span>  <span data-ttu-id="5ff02-149">I Q# returnerar en qubit-mätning alltid antingen `Zero` eller `One` .</span><span class="sxs-lookup"><span data-stu-id="5ff02-149">In Q#, a qubit measurement always returns either `Zero` or `One`.</span></span>  <span data-ttu-id="5ff02-150">Om mätningen returnerar ett värde som inte är lika med det önskade värdet, `SetQubitState` "vänder" qubit; som är, kör den en `X` åtgärd som ändrar qubit-tillstånd till ett nytt tillstånd där sannolikheten för en mätning returneras `Zero` och `One` blir omvänd.</span><span class="sxs-lookup"><span data-stu-id="5ff02-150">If the measurement returns a value not equal to the desired value, `SetQubitState` “flips” the qubit; that is, it runs an `X` operation, which changes the qubit state to a new state in which the probabilities of a measurement returning `Zero` and `One` are reversed.</span></span> <span data-ttu-id="5ff02-151">På så sätt `SetQubitState` placeras alltid mål qubit i önskat tillstånd.</span><span class="sxs-lookup"><span data-stu-id="5ff02-151">This way, `SetQubitState` always puts the target qubit in the desired state.</span></span>
...
#### <a name="about-no-locq-operations"></a><span data-ttu-id="5ff02-157">Om Q# åtgärder</span><span class="sxs-lookup"><span data-stu-id="5ff02-157">About Q# operations</span></span>

<span data-ttu-id="5ff02-158">En Q# åtgärd är en Quantum-underrutin.</span><span class="sxs-lookup"><span data-stu-id="5ff02-158">A Q# operation is a quantum subroutine.</span></span> <span data-ttu-id="5ff02-159">Det vill säga att det är en rutin som kan anropas och som innehåller anrop till andra Quantum-åtgärder.</span><span class="sxs-lookup"><span data-stu-id="5ff02-159">That is, it is a callable routine that contains calls to other quantum operations.</span></span>
...
<span data-ttu-id="5ff02-161">Returtypen för åtgärden anges efter ett kolon.</span><span class="sxs-lookup"><span data-stu-id="5ff02-161">The return type of the operation is specified after a colon.</span></span> <span data-ttu-id="5ff02-162">I det här fallet `SetQubitState` har åtgärden ingen returtyp, så den markeras som retur `Unit` .</span><span class="sxs-lookup"><span data-stu-id="5ff02-162">In this case, the `SetQubitState` operation has no return type, so it is marked as returning `Unit`.</span></span> <span data-ttu-id="5ff02-163">Detta är Q# detsamma `unit` som i F #, som är ungefär detsamma som `void` i C# och en tom tupel i python ( `()` som representeras av typ tipset `Tuple[()]` ).</span><span class="sxs-lookup"><span data-stu-id="5ff02-163">This is the Q# equivalent of `unit` in F#, which is roughly analogous to `void` in C#, and an empty tuple in Python (`()`, represented by the type hint `Tuple[()]`).</span></span>

<span data-ttu-id="5ff02-164">Du har använt två Quantum-åtgärder i din första Q# åtgärd:</span><span class="sxs-lookup"><span data-stu-id="5ff02-164">You have used two quantum operations in your first Q# operation:</span></span>
...
<span data-ttu-id="5ff02-188">I båda fallen härleds variabeltypen av kompileraren.</span><span class="sxs-lookup"><span data-stu-id="5ff02-188">In both cases, the type of a variable is inferred by the compiler.</span></span> <span data-ttu-id="5ff02-189">Q# kräver inte någon typ av kommentarer för variabler.</span><span class="sxs-lookup"><span data-stu-id="5ff02-189">Q# doesn't require any type annotations for variables.</span></span>
...
<span data-ttu-id="5ff02-196">För att kunna köra koden måste vi ange vilken kompilator *som* kan anropas för att köra kommandot när vi tillhandahåller `dotnet run` kommandot.</span><span class="sxs-lookup"><span data-stu-id="5ff02-196">In order to run the code we need to tell the compiler *which* callable to run when we provide the `dotnet run` command.</span></span> <span data-ttu-id="5ff02-197">Detta görs med en enkel ändring i Q# filen genom att lägga till en rad med `@EntryPoint()` direkt föregående anrop: `TestBellState` åtgärden i det här fallet.</span><span class="sxs-lookup"><span data-stu-id="5ff02-197">This is done with a simple change in the Q# file by adding a line with `@EntryPoint()` directly preceding the callable: the `TestBellState` operation in this case.</span></span> <span data-ttu-id="5ff02-198">Den fullständiga koden ska vara:</span><span class="sxs-lookup"><span data-stu-id="5ff02-198">The full code should be:</span></span>
...
<span data-ttu-id="5ff02-224">Nu ska vi titta på hur Q# uttrycker sätt att entangle qubits.</span><span class="sxs-lookup"><span data-stu-id="5ff02-224">Now let’s look at how Q# expresses ways to entangle qubits.</span></span>
...
<span data-ttu-id="5ff02-245">I självstudien om [Grover](xref:microsoft.quantum.quickstarts.search) får du lära dig hur du skapar och kör Grover search, en av de mest populära Quantum Computing-algoritmerna och erbjuder ett bra exempel på ett Q# program som kan användas för att lösa verkliga problem med Quantum Computing.</span><span class="sxs-lookup"><span data-stu-id="5ff02-245">The tutorial [Grover’s search](xref:microsoft.quantum.quickstarts.search) shows you how to build and run Grover search, one of the most popular quantum computing algorithms and offers a nice example of a Q# program that can be used to solve real problems with quantum computing.</span></span>  
