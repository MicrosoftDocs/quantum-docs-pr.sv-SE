...
## <a name="internal-declarations"></a>Interna deklarationer

Användardefinierade typer, åtgärder och funktioner kan också deklareras som *interna*.
Det innebär att de endast kan nås från det Q #-projekt som de är deklarerade i.
När ett projekt används som referens görs alla *offentliga* (icke-interna) deklarationer tillgängliga, men om du försöker använda en intern deklaration från ett annat projekt skapas ett fel.
Interna deklarationer är användbara för att skriva modulär kod som kan återanvändas av andra delar av projektet, men fortfarande ändras senare utan att du behöver bryta andra projekt som kan vara beroende av den.

En intern användardefinierad typ, åtgärd eller funktion kan deklareras genom att du lägger till `internal` i början av deklarationen.
Exempel:
...
> [!WARNING]
> Interna användardefinierade typer kan bara användas i signaturer eller underliggande typer om motsvarande anropande eller användardefinierade typ också är intern.
> Om det till exempel finns en användardefinierad typ `InternalOptions` som deklarerades med nyckelordet `internal`, kommer följande deklarationer leda till fel:
>
> ```qsharp
