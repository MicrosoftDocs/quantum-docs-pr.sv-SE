...
Vi är glada att kunna presentera den senaste uppdateringen av Quantum Development Kit.  Den här sidan ger en översikt över de nya funktionerna för Q #-språk och beskriver hur du [migrerar](#Migration) befintliga q #-program till den nya syntaxen.  Se vår fullständiga dokumentation för att få en fullständig beskrivning av språket för Q # [här](xref:microsoft.quantum.guide.basics).
...
Ett villkors uttryck är ett uttryck för formuläret `condition ? caseTrue | caseFalse` .
...
Två nya specialiserings direktiv för specialisering `invert` och `distribute` dessutom `auto` introduceras för att öka kontrollen över hur Functor-specialiseringar genereras.
En explicit deklaration av standard- `body` specialiseringen inom åtgärder är inte längre obligatorisk.
...
`Adjoint`, `Controlled` och en ny "unwrap" postfix-operator `!` behandlas som uttrycks modifierare. Genom `Adjoint` att behandla och `Controlled` som uttrycks modifierare elimineras behovet av parenteser för många fall. Uttrycken nedan är till exempel giltiga för `Op` , en enhetlig åtgärd, `opArr` en matris med enhetlig drift och `arg` ett lämpligt argument: 
```qsharp
    Adjoint Op (arg);
...
```
Som du kan se i exemplet är det mer `!` likt prefixs modifierare som `Controlled` och `Adjoint` .

## <a name="breaking-changes"></a><a name="BreakingChanges"></a>Bryta ändringar 
...
Förutom de ändringar som anges nedan `__*__` är mönstret där * är en sekvens av tecken som inte är blank steg reserverat för internt bruk och kan inte användas som symbol namn.  
...
Ett exempel på en sådan ändring är symbolens tupel i användardefinierade specialiseringar. I tidigare versioner har en kontrollerad eller kontrollerad specialiseringad specialisering angett ett enda symbol argument, med början i 0,3, vi rekommenderar att du anger en sådan symbol för alla specialiseringar `...` som anger att kompilatorn ska migrera symbolerna från den anropande deklarationen om det behövs. 
...
Följande exempel illustrerar hur du använder `ToOperation` funktionen för att ändra en funktion `Square` till en åtgärd `op` : 
```qsharp
...
## <a name="migrating-existing-projects-to-q-03"></a><a name="Migration"></a>Migrera befintliga projekt till Q # 0,3 
...
2. Redigera. CSPROJ-filen i en text redigerare och ändra versionen av alla "Microsoft. Quantum" `PackageReference` till version 0.3.1811.2802 – för hands version, till exempel:
```xml
...
## <a name="migration-script"></a><a name="MigrationScript"></a>Migrations skript 
